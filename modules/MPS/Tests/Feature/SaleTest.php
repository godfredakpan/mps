...
class SaleTest extends MPSTestCase
{
    ...
    public function testMPSCanCreatAndUpdateSale()
    {
    ...
    public function testMPSSaleValidation()
    {
        $response = $this->actingAs($this->user)->ajax()->post($this->route, []);
        $response->assertStatus(422);
        $response->assertJsonStructure(['message', 'errors']);
        $response->assertJsonValidationErrors(['date', 'customer_id', 'type', 'items']);

        $response2 = $this->actingAs($this->user)->ajax()->post($this->route, ['items' => [['id' => 'test']]]);
        $response2->assertStatus(422);
        $response2->assertJsonStructure(['message', 'errors']);
        $response2->assertJsonValidationErrors(['date', 'customer_id', 'type', 'items.0.code', 'items.0.name', 'items.0.price', 'items.0.quantity']);
    }
}
