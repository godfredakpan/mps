...
class QuotationPermissionsTest extends MPSTestCase
{
    use TestHelpers;
    ...
    protected function setUp(): void
    {
        ...
        $this->quotations = $this->createQuotation($this, 2, $this->super);
    }

    public function testMPS1SuperPermissionsForQuotationRoute()
    {
        $quotation = $this->quotations->first();

        // can access the view route
        $this->actingAs($this->super)->ajax()->get($this->route . $quotation->id)->assertOk();

        // can access the store route
        $this->actingAs($this->super)->ajax()->post($this->route, [])->assertStatus(422);

        // can access the update route
        $this->actingAs($this->super)->ajax()->put($this->route . $quotation->id, [])->assertStatus(422);

        // can access update route of others
        $this->actingAs($this->super)->ajax()->put($this->route . $quotation->id, [])->assertStatus(422);

        // can delete
        $this->actingAs($this->super)->ajax()->delete($this->route . $quotation->id)->assertOk();
    }

    public function testMPS2QuotationRolePermissionsForStaffWithNoPermissions()
    {
        app()[\Spatie\Permission\PermissionRegistrar::class]->forgetCachedPermissions();
        $quotation = $this->createQuotation($this, 1, $this->staff);
        $form      = $this->createQuotationForm($this, $this->staff, 1, null, $this->unit);

        $this->actingAs($this->staff)->ajax()->get($this->route . $quotation->id)->assertStatus(403);
        ...
        $this->actingAs($this->staff)->ajax()->put($this->route . $quotation->id, $form)->assertStatus(403);
        $this->actingAs($this->staff)->ajax()->delete($this->route . $quotation->id)->assertStatus(403);
    }

    public function testMPS3QuotationRolePermissionsForStaffWithPermissions()
    {
        app()[\Spatie\Permission\PermissionRegistrar::class]->forgetCachedPermissions();
        // quotations
        $role = $this->staff->roles()->first();
        Permission::firstOrCreate(['name' => 'read-quotations'])->assignRole($role);
        Permission::firstOrCreate(['name' => 'create-quotations'])->assignRole($role);
        Permission::firstOrCreate(['name' => 'update-quotations'])->assignRole($role);
        Permission::firstOrCreate(['name' => 'delete-quotations'])->assignRole($role);

        $quotation = $this->createQuotation($this, 1, $this->staff);
        $others    = $this->createQuotation($this, 1, $this->super);
        $form      = $this->createQuotationForm($this, $this->staff, 1, null, $this->unit);

        $this->actingAs($this->staff)->ajax()->get($this->route . '?limit=10&byColumn=false&orderBy=date+desc&page=1')->assertOk();
        $this->actingAs($this->staff)->ajax()->get($this->route . $quotation->id)->assertOk();
        $this->actingAs($this->staff)->ajax()->post($this->route, $form)->assertOk();
        $this->actingAs($this->staff)->ajax()->put($this->route . $quotation->id, $form)->assertOk();
        $this->actingAs($this->staff)->ajax()->delete($this->route . $quotation->id)->assertOk();
    ...
    public function testMPS4PublicCannotPerformAnyActionOnQuotation()
    {
