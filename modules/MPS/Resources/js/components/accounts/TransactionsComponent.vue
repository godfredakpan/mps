<template>
  <div>
    ...
    <Modal :title="$tc('income')" width="750" v-model="view_income" :footer-hide="true" class="np-header-footer">
      <div v-if="loading" class="py16">
        <Loading />
      </div>
      <income-view-component :income="income" />
    </Modal>
    <Modal :title="$tc('expense')" width="750" v-model="view_expense" :footer-hide="true" class="np-header-footer">
      <div v-if="loading" class="py16">
        <Loading />
      </div>
      <expense-view-component :expense="expense" />
    </Modal>
  </div>
</template>
...
export default {
  components: { ExpenseViewComponent, IncomeViewComponent, PaymentViewComponent, ReportFormComponent },
  data() {
    return {
      url: '',
      data: {},
      ...
      view_expense: false,
      view_payment: false,
      fields: ['created_at', 'customer_id', 'supplier_id', 'user_id'],
      columns: [
        {
          width: 50,
          type: 'expand',
        ...
        { title: this.$tc('account'), sortable: false, key: 'account', minWidth: 200, render: this.renderAccount },
        { title: this.$tc('subject'), sortable: false, key: 'subject', minWidth: 200, render: this.renderSubject },
  ...
  created() {
    ...
    this.url = `app/accounts/transactions/${this.$route.params.id}/table`;
    this.$http
      .get(`app/accounts/transactions/${this.$route.params.id}`, { params: { ...this.reportForm, date: '' } })
      .then(res => {
  ...
  methods: {
    handleSubmit(form) {
      ...
      this.url = `app/accounts/transactions/${this.$route.params.id}/table?${this.queryString(this.reportForm)}`;
      this.refresh++;
      this.$http
        .get(`app/accounts/transactions/${this.$route.params.id}`, { params: { ...this.reportForm, date: '' } })
        .then(res => {
    ...
    showInfo(row) {
      ...
      } else if (row.subject_type && row.subject_type.includes('Expense')) {
        this.viewExpense(row.subject_id);
    ...
    renderAccount(h, params) {
      return (
        <div>{params.row.journal.morphed ? params.row.journal.morphed.name + ' (' + params.row.journal.morphed.reference + ')' : ''}</div>
      );
    },
    renderSubject(h, params) {
      if (params.row.subject_type) {
        let type = params.row.subject_type.split('\\');
        return <div>{type[type.length - 1] + ': ' + params.row.subject_id}</div>;
      }
    },
    renderCredit(h, params) {
      return <div class="text-right">{this.formatJournalBalance(params.row.credit, this.$store.getters.settings.decimals)}</div>;
    },
    renderDebit(h, params) {
      return <div class="text-right">{this.formatJournalBalance(params.row.debit, this.$store.getters.settings.decimals)}</div>;
    },
    ...
    viewIncome(id) {
      this.view_income = true;
      if (id != this.income.id) {
        this.loading = true;
        this.$http
          .get(`app/incomes/${id}`)
    ...
    viewExpense(id) {
      this.view_expense = true;
      if (id != this.expense.id) {
        this.loading = true;
        this.$http
          .get(`app/expenses/${id}`)
