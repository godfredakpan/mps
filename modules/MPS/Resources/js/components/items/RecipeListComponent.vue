<template>
  <div>
    <Card :dis-hover="true">
      <p slot="title">{{ $tc('recipe', 2) }}</p>
      <router-link to="/items/recipes/add" slot="extra">
        <Icon type="ios-grid-outline" />
        {{ $t('add_x', { x: $tc('recipe') }) }}
      </router-link>
      <div>
        <table-component
          url="app/items?type=recipe"
          :columns="columns"
          :options="options"
          :dblClickCB="showInfo"
...
export default {
  ...
  mixins: [Table('item', 'app/items', 'name', 'code', 'recipes')],
  data() {
    return {
      row: {},
      view: false,
      ...
      columns: [
        ...
        { title: this.$tc('portion', 2), sortable: false, key: 'portions', minWidth: 200, render: this.renderPortion },
  ...
  methods: {
    ...
    renderPortion(h, params) {
      return <div>{params.row.portions.map(c => this.$t(c.name)).join(', ')}</div>;
    },
