  ...
  <div class="order-item">
    <span class="item">
      ...
      <span class="remove">
        ...
        <span v-if="row.selected.variations && row.selected.portions && !row.selected.portions.length && !row.selected.variations.length">
          <Icon size="18" type="md-close" class="pointer" color="#ed4014" @click="remove(row)" />
        </span>
      </span>
      <span class="details pointer" @click="editItem(index)">
        ...
        <span>
          <span
            ...
            "
          ></span>
          <span v-if="row.promotions && row.promotions.length"
            >{{ $tc('promo', row.promotions.length) }}:
            {{
              row.allPromotions
                .filter(p => row.promotions.includes(p.id))
                .map(p => p.name + ' (' + formatNumber(p.discount) + ')')
                .join(',')
            }}
          </span>
      ...
      <template v-if="row.selected.variations && row.selected.portions && !row.selected.portions.length && !row.selected.variations.length">
        <span class="price">
          {{ formatNumber(row.price - row.discount_amount + calcItemTax(row)) }}
        </span>
    ...
    <template v-if="row.selected.variations && row.selected.variations.length">
      <div class="combo-item variation" :key="'svi_' + svi" v-for="(sv, svi) in row.selected.variations">
        ...
        <span class="subtotal">
          {{ formatNumber((sv.price - sv.discount_amount + sv.total_tax_amount) * sv.quantity) }}
        </span>
      </div>
    </template>
      ...
      <div class="combo" v-for="(sp, spi) in row.selected.portions" :key="'spi_' + spi">
        <div class="combo" v-for="p in row.portions.filter(ip => ip.id == sp.id)" :key="'pi_' + p.id">
          <div class="combo-item variation">
            <span class="index"></span>
            <span class="remove">
              <Icon size="18" class="pointer" color="#ed4014" type="md-close" @click="remove(row, 'Portion', sp, spi)" />
            </span>
            <span class="details leading-medium">
              <p>
                {{ spi + 1 }}. {{ $tc('portion') }}:
            ...
            <span class="price">{{ formatNumber(sp.price) }}</span>
            <span class="discount">
          ...
          <div :key="'ei_' + ei" class="combo-item bt0" style="padding-top: 1px;" v-for="(e, ei) in p.portion_items">
            <span class="index"></span>
            <span class="remove"></span>
            <span class="details">
              #{{ ei + 1 }} {{ e.item.name }}
              <span v-if="sp.portion_items[ei].variation_id"> (<span v-html="metaString(sp.portion_items[ei].meta)"></span>) </span>
            </span>
            <span class="price"></span>
            <span class="discount"></span>
          ...
          <div :key="'ei_' + ei" class="combo-item bt0" style="padding-top: 1px;" v-for="(e, ei) in p.essentials">
            ...
            <span class="details">
              #{{ ei + 1 }} {{ e.item.name }}
              <span v-if="sp.essentials[ei].variation_id"> (<span v-html="metaString(sp.essentials[ei].meta)"></span>) </span>
            </span>
            <span class="price"></span>
            <span class="discount"></span>
            <span class="quantity input">
          ...
          <template v-for="(g, gi) in p.choosables">
            ...
            >
              ...
              <span class="details">
                ...
                {{ gitem.item.name }}
                <span v-if="sp.choosables[gi].variation_id"> (<span v-html="metaString(sp.choosables[gi].meta)"></span>) </span>
              </span>
              <span class="price"></span>
              <span class="discount"></span>
              <span class="quantity input">
    ...
    <template v-if="row.selected.modifiers.length">
      ...
      <div :key="'mi_' + mi" v-for="(m, mi) in row.selected.modifiers">
        <div class="combo-item pt pb">
          ...
          <span class="subtotal">
            <!-- {{ formatNumber((m.price - m.discount_amount + m.total_tax_amount) * m.quantity) }} -->
          </span>
        </div>
      </div>
    </template>
...
export default {
  props: {
    ...
    getPortionChoosable: { type: Function },
