<template>
  <div>
    <Card :dis-hover="true">
      <p slot="title">{{ form.id ? $t('edit') : $t('add') }} {{ $tc('sale') }}</p>
      <router-link to="/sales" slot="extra">
        <Icon type="ios-grid-outline" />
        {{ $t('list') }} {{ $tc('sale', 2) }}
      </router-link>
      <div>
        <Form ref="sale" :model="form" :rules="rules" :label-width="150" class="form-responsive">
          <Row :gutter="16">
            <Col :sm="24" :md="24" :lg="24">
              ...
              <FormItem>
                <Button type="primary" :loading="saving" :disabled="saving" @click="handleSubmit('sales')">
...
export default {
  ...
  mixins: [Order('sale', null, null, { id: 'sale_id', qId: 'quotation_id' }), OrderHelpers('price', 'form', false)],
};
</script>
