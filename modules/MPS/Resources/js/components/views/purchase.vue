<template>
  <div class="fullpage">
    ...
    <div v-else class="order-view">
      <order-view-component field="cost" to="supplier" :record="purchase" :heading="$tc('purchase')" :to-text="$tc('order_to')" />
    </div>
  </div>
</template>
...
export default {
  ...
  created() {
    if (this.$route.params.hash) {
      this.loading = true;
      this.$http
        .get(`/views/purchase/${this.$route.params.hash}`)
        .then(res => {
          this.purchase = res.data;
          this.$nextTick(() => {
            document.title = this.$tc('purchase') + ' ' + this.purchase.reference;
          });
        })
        .catch(err => {
