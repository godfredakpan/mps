<template>
  <div class="fullpage">
    ...
    <div v-else class="order-view">
      <order-view-component :summary="false" :record="quotation" :to-text="$tc('quote_to')" :heading="$tc('quotation')" />
    </div>
  </div>
</template>
...
export default {
  ...
  created() {
    if (this.$route.params.hash) {
      this.loading = true;
      this.$http
        .get(`/views/quotation/${this.$route.params.hash}`)
        .then(res => {
          this.quotation = res.data;
          this.$nextTick(() => {
            document.title = this.$tc('quotation') + ' ' + this.quotation.reference;
          });
        })
        .catch(err => {
