<template>
  <div class="fullpage">
    ...
    <div v-else class="order-view">
      <order-view-component :record="sale" :heading="$tc('sale')" :to-text="$tc('bill_to')" />
    </div>
  </div>
</template>
...
export default {
  ...
  created() {
    if (this.$route.params.hash) {
      this.loading = true;
      this.$http
        .get(`/views/sale/${this.$route.params.hash}`)
        .then(res => {
          this.sale = res.data;
          this.$nextTick(() => {
            document.title = this.$tc('sale') + ' ' + this.sale.reference;
          });
        })
        .catch(err => {
