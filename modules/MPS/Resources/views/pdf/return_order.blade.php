...
<head>
  ...
  <title>{{ __('Return Order No.') }}: {{ $return_order->number }}</title>
...
<body>
  <div class="max-w-3xl mx-auto text-gray-700 text-sm whitespace-normal break-normal">
    <div class="w-full pb-2 text-right">
      <img src="{{ $return_order->location->logo ?? $settings['default_logo'] }}" alt="{{ __($return_order->location->name) }}"
        class="max-h-20 w-48 float-left" />
      <div class="inline-block text-right ml-48">
        <div class="float-right">
          {!! generate_barcode($return_order->reference, null, 1, 70, 'black') !!}
        </div>
    ...
    <div class="mt-2 py-2 text-center uppercase font-bold border-b-2 border-t-2">
      {{ trans_choice('return_order', 1) }}
      ({{ trans_choice($return_order->type, 1) }})
    </div>
    <div class="py-2 border-b">
      <div class="left">
        <div class="pr-2">
          <div>{{ __('date') }}: <span class="font-bold">{{ $return_order->date->toFormattedDateString() }}</span>
          </div>
          <div>{{ __('created_at') }}: <span class="font-bold">{{ $return_order->created_at->toDayDateTimeString() }}</span>
          </div>
        </div>
      </div>
      <div class="right">
        <div class="pl-2">
          <div>{{ __('Reference') }}: <span class="font-bold">{{ $return_order->reference }}</span></div>
          <div>{{ __('id') }}: <span class="font-bold">{{ $return_order->id }}</span></div>
        </div>
      </div>
      <div class="clearfix"></div>
    </div>
    @unless($return_order->paid)
      @if ($return_order->draft)
    ...
    <div class="py-4">
      <div class="left">
        <div class="pr-2 whitespace-normal break-words">
          ...
          <strong>{{ $return_order->location->name }}</strong><br />
          {{ $return_order->location->address }}
          {{ $return_order->location->state_name }}
          {{ $return_order->location->country_name }}<br />
          {{ $return_order->location->phone }}<br />
          {{ $return_order->location->email }}<br />
        </div>
      </div>
      <div class="right">
        <div class="pl-2 whitespace-normal break-words">
          @if ($return_order->supplier)
            <div class="text-sm">{{ __('to') }}:</div>
            <strong>{{ $return_order->supplier->name }}</strong><br />
            {{ $return_order->supplier->address }}
            {{ $return_order->supplier->state_name }}
            {{ $return_order->supplier->country_name }}<br />
            {{ $return_order->supplier->phone }}<br />
            {{ $return_order->supplier->email }}<br />
          @else
            <div class="text-sm">{{ __('from') }}:</div>
            <strong>{{ $return_order->customer->name }}</strong><br />
            {{ $return_order->customer->address }}
            {{ $return_order->customer->state_name }}
            {{ $return_order->customer->country_name }}<br />
            {{ $return_order->customer->phone }}<br />
            {{ $return_order->customer->email }}<br />
          @endif
        </div>
      </div>
      <div class="clearfix"></div>
              ...
              <tfoot class="stripe divide-y divide-gray-200 text-gray-900">
                <tr>
                  ...
                  <td class="px-4 py-2 font-bold text-right">
                    {{ formatNumber($return_order->total) }}
                  </td>
                </tr>
                @if ($return_order->total_tax_amount)
                  <tr>
                    <td colspan="3" class="px-4 py-2 text-gray-900">
                      {{ __('Total Tax Amount') }}
                    </td>
                    <td class="px-4 py-2 font-bold text-right">
                      {{ formatNumber($return_order->total_tax_amount) }}
                    </td>
                  </tr>
                @endif
                @if ($return_order->total_discount_amount)
                  <tr>
                    <td colspan="3" class="px-4 py-2 text-gray-900">
                      {{ __('Total Discount Amount') }}
                    </td>
                    <td class="px-4 py-2 font-bold text-right">
                      {{ formatNumber($return_order->total_discount_amount) }}
                    </td>
                  </tr>
                @endif
                @if ($return_order->shipping)
                  <tr>
                    <td colspan="3" class="px-4 py-2 text-gray-900">
                      {{ __('Shipping') }}
                    </td>
                    <td class="px-4 py-2 font-bold text-right">
                      {{ formatNumber($return_order->shipping) }}
                    </td>
                  </tr>
                @endif
                <tr>
                  ...
                  <td class="px-4 py-2 font-bold text-right">
                    {{ formatNumber($return_order->grand_total) }}
                  </td>
                </tr>
                @if ($return_order->payments->isNotEmpty())
                  @foreach ($return_order->payments as $payment)
                    <tr>
                  ...
                  <tr>
                    ...
                    <td class="px-4 py-2 font-bold text-right">
                      {{ formatNumber($return_order->grand_total - ($return_order->payments->isNotEmpty() ? $return_order->payments->reduce(fn($a, $p) => $a + $p->amount, 0) : 0)) }}
                    </td>
                  </tr>
                @else
                  <tr>
                  ...
                  <tr>
                    <td colspan="3" class="px-4 py-2">{{ __('Balance Due') }}</td>
                    <td class="px-4 py-2 font-bold text-right">
                      {{ formatNumber($return_order->grand_total) }}</td>
                  </tr>
                @endif
              </tfoot>
            </table>
    ...
    @php
      $taxSummary = \Modules\MPS\Actions\TaxSummary::calculate($return_order, true);
    @endphp

    @if ($settings['show_tax_summary'] && $return_order->total_tax_amount && $taxSummary->isNotEmpty())
    ...
    @if ($return_order->details)
      <div class="py-4">
        {{ __($return_order->details) }}
      </div>
    @endif
